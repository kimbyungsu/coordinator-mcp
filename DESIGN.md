# Multi-Agent Coordination MCP — 설계 문서 v0.2

> v0.1 → v0.2 주요 변경
> - 구현/검증 모드에서 강제 타협 제거 (severity 기반 제한으로 교체)
> - 프로젝트 완료 판정을 추론형 → 체크리스트형으로 변경
> - 단일 writer lock 운영 강제 장치 명문화
> - Verifier 입력에 diff/patch 근거 체인 추가
> - 총 사이클/턴 예산 상한 추가
> - Memento 중간 동기화 기준 강화 (stable=true 조건)
> - 인접 파일 수정 허용 프로토콜 추가
> - Coordinator 역할 = 상태 관리자로 제한 (설계 의사결정자 금지)
> - Issue ID 발급 주체 Coordinator로 일원화
> - confidence = 참고 지표로 격하 (단독 교착 트리거 불가)
> - PAUSED / RESTORING / RESYNCING / READY_TO_RESUME 상태 추가
> - 체크포인트 복원 프로토콜 전면 추가 (10장)
> - Obligation 생애주기 상태 세분화
> - 용어/섹션 번호/상태 명칭 통일
...
[사용자]
   ...
   ▼
[Coordinator MCP]
   ├── read/write ──→ [Agent A: Implementer]
   └── read/write ──→ [Agent B: Verifier]
          (DISCUSSION 모드: A=Proposer, B=Critic)
```
...
- [ ] acceptance_criteria 항목 전부 체크됨
- [ ] open_obligations 목록이 비어 있음
- [ ] 사용자의 명시적 완료 승인 (선택 사항, 설정에 따라)

위 조건 전부 만족 시에만 DONE 전환 가능.
...
- Verifier는 issue_draft를 제출하고, Coordinator가 canonical issue_id를 부여한다.
- 이후 모든 상태 전이는 Coordinator가 관리한다.
- ID 형식: `{session_id}-V{순번}` (예: `sess001-V003`)

### 4.2 Obligation 생애주기 상태
```
proposed      → Verifier가 이슈 draft 제출
accepted      → Coordinator가 ID 부여 + Implementer가 수용 의사 표명
in_progress   → Implementer가 현재 해당 이슈 처리 중
resolved      → Implementer가 처리 완료 + Verifier가 실제 코드 기준으로 확인
deferred      → 양쪽 합의 하에 다음 task로 연기 (low severity만 허용)
rejected      → Implementer가 반박 근거 제시, Verifier가 수용
```

전이 규칙:
- `proposed → accepted`: Coordinator가 ID 부여 시 자동 전환
- `accepted → in_progress`: Implementer 선언 시
- `in_progress → resolved`: Verifier가 실제 코드에서 근거 확인 후만 가능
- `accepted/in_progress → deferred`: severity=low + 양쪽 동의 + Coordinator 승인
- `accepted/in_progress → rejected`: Implementer 반박 + Verifier 수용
- **high/blocking severity 이슈는 deferred 전환 불가**
...
[Turn N]
  Phase A: 구현
    Coordinator → Implementer 주입:
      ...
      - open_obligations 목록 (in_progress/accepted 상태인 항목)
      - 이전 협의 agreed_actions

  Phase B: 구현 요약 + diff 전달
    Implementer → Coordinator 제출 (Section 6 RULE 4 형식)
    Coordinator → Verifier 전달 + issue ledger 첨부
  ...
  Phase D: 협의 (FIX_REQUIRED 시 진입)
    - severity=high/blocking: 타협 불가, 해결 또는 ESCALATED
    - severity=low: deferred 가능 (양쪽 동의 + Coordinator 승인)
    - 최대 max_negotiation_turns_per_cycle 턴
    - 초과 시 → high/blocking 미해결이면 ESCALATED
                low만 남았으면 모두 deferred 처리 후 다음 Phase A

  Phase E: 합의 완료
    Coordinator: agreed_actions + deferred 목록 기록
    예산 체크: 총 턴/사이클 상한 초과 여부 확인
    → 다음 Turn Phase A 시작 또는 DONE/ESCALATED
```

### 5.2 예산 체크 (매 Phase E 종료 시)
```
if current_verification_cycles >= max_verification_cycles_per_task:
    → ESCALATED ("task 내 최대 검증 사이클 초과")

if total_turns >= max_total_turns_per_session:
...
### RULE 4: 검증 모델 전달 요약 + diff 제출 의무 [HARD]
...
### RULE 5: 판정 형식 고정 + issue draft 제출 [HARD]
...
```json
{
  ...
  "previous_unresolved": ["<미해결 issue_id 목록>"],
  "retest_focus": ["<다음 턴 재확인 항목>"]
}
```

판정 기준:
- **PASS**: 모든 blocking 항목 resolved + acceptance_criteria 항목 충족
...
### 진입 조건
Verifier 판정이 FIX_REQUIRED일 때

### severity별 처리 규칙
```
severity=high 또는 blocking=true:
  → 타협 불가
  → 해결(resolved) 또는 ESCALATED만 가능

severity=medium:
  → 반박 가능, 합의 시 deferred 가능
  → 단, 타협안(강제 절충) 형태로 진행 불가

severity=low:
  → deferred 허용 (양쪽 동의 + Coordinator 승인)
```
...
```
정상 종료:
  모든 high/blocking 이슈 → accepted 또는 rejected (합의)
  남은 이슈 전부 deferred (low만 해당)

비정상 종료 (ESCALATED):
  max_negotiation_turns_per_cycle 초과 + high/blocking 미해결
  → Coordinator: ESCALATED 전환 + 사용자 보고
```

**DISCUSSION 모드에서만 타협안(forced_compromise) 허용.**
PROJECT/SINGLE 모드에서는 forced_compromise 생성 불가.
...
IMPLEMENTING
  → 구현 요약 + diff 제출 완료: VERIFYING

VERIFYING
  → PASS + 완료 조건 미충족 (PROJECT): NEXT_TASK
  → PASS + 완료 조건 충족 (PROJECT): DONE
  → PASS (SINGLE): DONE
...
RESTORING
  → Coordinator가 체크포인트 로드
  → 복원 패키지 구성 완료: RESYNCING

RESYNCING
  → Coordinator가 각 에이전트에 복원 패키지 주입
  → 에이전트 ACK 수신 대기
  → 모든 에이전트 ACK 확인: READY_TO_RESUME
  → ACK 타임아웃: PAUSED 재진입

READY_TO_RESUME
  → Coordinator: 직전 중단 상태(IMPLEMENTING/VERIFYING/NEGOTIATING)로 복귀
  → 복귀 전 사용자에게 "재개 준비 완료" 알림 (선택)
...
3. `max_verification_cycles_per_task` 초과
4. `max_total_turns_per_session` 초과
...
진행 턴: {총 턴} / 예산: {max_total_turns}
검증 사이클: {현재 사이클} / 예산: {max_verification_cycles_per_task}
...
- 세션 재시작 감지
- 에이전트 컨텍스트 손실 감지 (이전 turn 참조 불가 신호)
- 컨텍스트 압축 실행 후
...
```json
{
  ...
  "your_write_permission": true,
  "open_obligations": ["<의무 목록>"],
...
```json
{
  ...
  "questions": ["<불명확한 점 있으면 기재, 없으면 빈 배열>"]
}
```

questions가 비어 있지 않으면 Coordinator가 답변 후 재ACK 요청.

### 11.6 컨텍스트 압축 규칙
누적 턴이 임계값 초과 시:
- **보존**: open_obligations 전체, unresolved_issues 전체, 최근 3턴 전문, project_brief, acceptance_criteria 상태
- **압축**: 해결된 이슈 상세 내용, 오래된 협의 내용, closed 상태 obligations 상세
- **압축 후**: RESTORING → RESYNCING → READY_TO_RESUME 사이클 강제 실행

### 11.7 Memento 중간 동기화 기준
`memento_sync_interval_turns` 도달 시 아래 조건 만족 항목만 Memento에 동기화:

```
stable=true 조건:
  - PASS 판정을 받은 구현 요약
  - 3회 이상 반복된 이슈 패턴 (같은 category)
  - 양쪽 모두 accepted 상태인 합의 규칙
...
4. 에이전트 응답 수신 + 형식 검증
5. issue_drafts 수신 시 canonical issue_id 부여 + Ledger 업데이트
6. 교착 조건 체크
7. 상태 전이 판정
8. 체크포인트 저장 조건 확인
...
*v0.2 — 2026-04-15*
*v0.1 → v0.2: 13개 피드백 + 체크포인트 복원 프로토콜 전면 반영*
*다음 단계: v0.2 검토 → v0.3 (SQLite 스키마 + MCP 도구 목록)*
